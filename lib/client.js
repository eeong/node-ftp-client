...
Client.prototype.upload = function (patterns, dest, options, exceptWord, uploadCallback ) {
    ...
    paths = this._stat(paths, exceptWord);
    files = paths[1];
    dirs = paths[0];
            ...
            if (options.overwrite === 'all') {
                ...
                async.eachLimit(files.concat(dirs), MAX_CONNECTIONS, function (file, callback) {
                    ...
                    ftp.list(destPath, function (err, list) {
                        ...
                        if (list && list[0]) {
                            if (options.overwrite === 'older' && list[0].size && list[0].size !== this.size) {
                                toDelete.push(this);
                            } else {
                                if (this.isDirectory()) {
...
Client.prototype._stat = function (files, exceptWord) {
    var result = [
        [],
        []
    ];
    _.each(files, function (file) {
        if(file.indexOf(exceptWord) === -1) {
